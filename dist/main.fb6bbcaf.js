...
function fetchChampions() {
  fetch("https://ddragon.leagueoflegends.com/cdn/13.18.1/data/es_ES/champion.json").then(function (response) {
    return response.json();
  }).then(function (data) {
    for (var key in data.data) {
      var champion = data.data[key];
      var lore = new _Lore.default(champion);
      heroes.push(lore);
    }
    renderCards(); // Llama a la función para renderizar
  }).catch(function (err) {
    return console.error("Error fetching champions:", err);
  });
}
